...
3. **Gather implementation evidence**:
   ...
   if git diff HEAD~N..HEAD --name-only | grep -q "^api/"; then
       cd api && uv run pytest && uv run pyright && uv run ruff check . && cd ..
   fi

   # For IaC changes
   if git diff HEAD~N..HEAD --name-only | grep -q "^iac/"; then
       cd iac && terraform validate && terraform fmt -check && cd ..
   fi
...
✓ Client build passes: `cd client && pnpm build`
✓ API tests pass: `cd api && uv run pytest`
✗ Linting issues: `cd api && uv run ruff check .` (3 warnings)
