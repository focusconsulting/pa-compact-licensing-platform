...
3. **Set up the remote and worktree**:
   ...
   - If not, add it: `git remote add USERNAME git@github.com:USERNAME/pa-compact-platform`
   - Fetch from the remote: `git fetch USERNAME`
   - Create worktree: `git worktree add -b BRANCHNAME ~/wt/pa-compact-platform/SHORT_NAME USERNAME/BRANCHNAME`

4. **Configure the worktree**:
   - Copy Claude settings: `cp .claude/settings.local.json WORKTREE/.claude/`
   - Install dependencies: `cd WORKTREE/api && uv sync && cd ../client && pnpm install`
...
```
/local_review colleague:colleague/feature-branch-name
```

This will:
- Add 'colleague' as a remote
- Create worktree at `~/wt/pa-compact-platform/<short-name>`
